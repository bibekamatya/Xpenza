...
const Filters = ({ onApplyFilters }: FiltersProps) => {
  ...
  const handleSearchChange = (value: string) => {
    ...
    searchTimeoutRef.current = setTimeout(() => {
      onApplyFilters({ type, dateRange, category, search: value, startDate, endDate });
    }, 500);
  };
  ...
  const handleClear = () => {
    ...
    setStartDate("");
    setEndDate("");
    onApplyFilters({ type: "all", dateRange: "all", category: "all", search: "", startDate: "", endDate: "" });
  };

  const activeFiltersCount = [type !== "all", dateRange !== "all", category !== "all", search, startDate, endDate].filter(Boolean).length;
        ...
        <div className="flex items-center gap-3">
          <div className="flex gap-2">
            {["all", "income", "expense"].map((t) => (
              <button
                ...
                onClick={() => {
                  setType(t);
                  onApplyFilters({ type: t, dateRange, category, search, startDate, endDate });
                }}
                className={`px-4 h-[42px] text-sm rounded-lg transition-all font-medium whitespace-nowrap ${
                  type === t ? "bg-blue-600/20 text-blue-400 border border-blue-600/30" : "bg-slate-800 border border-slate-700 text-slate-300 hover:bg-slate-700"
                }`}
              >
          ...
          <select
            ...
            onChange={(e) => {
              ...
              onApplyFilters({ type, dateRange, category: e.target.value, search, startDate, endDate });
            }}
            ...
            >
              ...
              {(dateRange !== "all" || startDate || endDate) && (
                <span className="absolute -top-1 -right-1 w-5 h-5 bg-blue-600 text-white text-xs rounded-full flex items-center justify-center">
                  {[dateRange !== "all", startDate, endDate].filter(Boolean).length}
                </span>
              )}
            </button>
            {showDropdown && (
              <div className="absolute right-0 mt-2 w-96 bg-slate-800 border border-slate-700 rounded-xl shadow-xl z-10 p-5 space-y-4">
                    ...
                    {["all", "today", "week", "month", "year"].map((range) => (
                      <button
                        ...
                        className={`px-3 py-2 text-xs rounded-lg transition-all font-medium ${
                          dateRange === range ? "bg-blue-600 text-white" : "bg-slate-700 text-slate-300 hover:bg-slate-600"
                        }`}
                      >
                ...
                <div>
                  <label className="text-xs font-medium text-slate-400 mb-2 block">Custom Date Range</label>
                ...
                <div className="flex gap-2 pt-3 border-t border-slate-700">
                  <button
                    onClick={() => {
                      ...
                      setStartDate("");
                      setEndDate("");
                      onApplyFilters({ type, dateRange: "all", category, search, startDate: "", endDate: "" });
                    }}
        ...
        <div className="flex flex-wrap gap-2 mt-3">
          {type !== "all" && (
            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-600/20 text-blue-400 text-xs rounded-full font-medium">
              ...
              <button onClick={() => { setType("all"); onApplyFilters({ type: "all", dateRange, category, search, startDate, endDate }); }} className="hover:bg-blue-600/30 rounded-full p-0.5 transition-colors">
                <X className="w-3 h-3" />
              </button>
            </span>
          )}
          {dateRange !== "all" && (
            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-600/20 text-blue-400 text-xs rounded-full font-medium">
              ...
              <button onClick={() => { setDateRange("all"); onApplyFilters({ type, dateRange: "all", category, search, startDate, endDate }); }} className="hover:bg-blue-600/30 rounded-full p-0.5 transition-colors">
                <X className="w-3 h-3" />
              </button>
            </span>
          )}
          {category !== "all" && (
            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-600/20 text-blue-400 text-xs rounded-full font-medium">
              {category}
              <button onClick={() => { setCategory("all"); onApplyFilters({ type, dateRange, category: "all", search, startDate, endDate }); }} className="hover:bg-blue-600/30 rounded-full p-0.5 transition-colors">
                <X className="w-3 h-3" />
              </button>
            </span>
          )}
          {(startDate || endDate) && (
            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-600/20 text-blue-400 text-xs rounded-full font-medium">
              ...
              <button onClick={() => { setStartDate(""); setEndDate(""); onApplyFilters({ type, dateRange, category, search, startDate: "", endDate: "" }); }} className="hover:bg-blue-600/30 rounded-full p-0.5 transition-colors">
                <X className="w-3 h-3" />
              </button>
            </span>
          )}
